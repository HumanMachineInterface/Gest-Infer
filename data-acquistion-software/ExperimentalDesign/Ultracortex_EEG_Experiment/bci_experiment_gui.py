...
def acquire_data():
    ...
    seconds = int(acquisition_time.get())
    subject_index = subject_no.get()
    experiment_index = clicked.get().split()[1]
    repetition_index = rep_no.get()

    csv_file_path = base_path + "/BMIS_EEG_DATA/data/csv_data/subject_" + subject_index
    mat_file_path = base_path + "/BMIS_EEG_DATA/data/mat_data/subject_" + subject_index
    ...
    wait_seconds = 3
    get_eeg_data(wait_seconds, seconds, csv_file, mat_file)
...
def display_gesture_new_window():
    ...
    # be treated as a neachttps://docs.xilinx.com/r/2.0-English/ug1414-vitis-ai/Running-vai_q_tensorflow2dow
    new_window = tk.Toplevel(window)

    # sets the title of the
    # Toplevel widget
    new_window.title("Gesture Display")
    ...
    image_label = ttk.Label(new_window, image=images, width=100, padding=100)
    image_label.grid(column=200, row=200)
    new_window.mainloop()


window = tk.Tk()
window.title("EEG DATA ACQUISITION SOFTWARE")
...
experiment_message = ttk.Label(text="EEG EXPERIMENT")
...
image_list = ["exp{}.jpg".format(i) for i in range(1, 8)]
